...
CREATE OR REPLACE FUNCTION public.get_ranking(p_date date DEFAULT NULL::date, p_group_id uuid DEFAULT NULL::uuid)
  ...
  FOR v_rec IN
    EXECUTE format($q$
      ...
      relevant_matches AS (
        ...
        WHERE (
          m.is_finished = true
          OR (m.is_started = true AND m.is_finished = false)
        )
        AND (
          $1::date IS NULL
